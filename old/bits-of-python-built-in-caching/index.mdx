...
description:
  Easily squeeze performance out of your scripts, with only slight modifications. Learn how
  to leverage caching easily in Python.
readMinutes: 10
...
<BitsOfSummary>
  <BitsOfNutshell>
    In a nutshell: caching is an easy way to improve performance when costly computations can be
    reused. You can implement it easily in Python using @cache and @lru_cache decorators
  </BitsOfNutshell>
  <BitsOfInfoWarnSection>
    <BitsOfInfoBlock>
      Costly repeated computations& Real-time performance requirements
    </BitsOfInfoBlock>
    <BitsOfWarnBlock>
      Memory can blow up if the cache size in unbounded& Freshness can be an issue in some cases
    </BitsOfWarnBlock>
  </BitsOfInfoWarnSection>
</BitsOfSummary>
...
not the most performant implementaion. The iterative version is both more computationally and memory
efficient. However it's a good example to demonstrate the benefits of caching.
...
```python
---
{"filename": "cache.py", "github": "https://github.com/bitswired/bitsof-python/blob/master/bitsof-python/caching/cache.py" }
---
...
```python
---
{"filename": "cache.py", "github": "https://github.com/bitswired/bitsof-python/blob/master/bitsof-python/caching/cache.py", "collapsable": true }
---
...
[github repo]: https://github.com/bitswired/bitsof-python/blob/master/bitsof-python/caching/cache.py
