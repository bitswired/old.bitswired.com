...
description:
  Python is great but lacks structure. I detail the issues and provide a GitHub template repository
  to solve the issue
readMinutes: 20
...
images:
  - https://statics.bitswired.com/images-opti/blog/code.webp
---

## TLDR

_Python is a great language with a lot of libraries available. From gaming to artificial
...
different versions, you don't want them to conflict with each other.
...
- **Dependency mix**: you get a `requirements.txt` file mixed with your dependencies as well as
  pacakges required by these dependencies. It's a mess to track easily the packages you really
  depend on
...
- **Strong reproducibility**: poetry introduces a **lock file** `poetry.clock` to your project. In
...
A good solution here cover 5 points.
...
newlines between functions and so on. Isort will help you keep your imports sorted with seperation
betwwen python built-ins, first-party and third-party dependencies. It's better to keep it
consistent accorss all files in your project.

<Figure
  ratio={16 / 9}
  width="100%"
  maxWidth="500px"
  src="https://statics.bitswired.com/images-opti/blog/python-formatting-before-after@blog.webp"
...
want to run all your code qualtiy checks before any code is merged to your master branch to avoid
...
1. Pre-commit hooks: these are scripts run before each commit locally, they will run all your code
   quality tools and deny the commit if the code doe not pass. This is an easy way to enforce
   best-practices at every commit
...
locally, you don't need to wait for GitHub remotely. However, nothiung guarantees you that every
developper run the pre-commit hooks before pushing code. So having the GitHub actions checking your
code is the ultimate gurantee that all code merged on master lives up to your quality standards

In the solution proposed here, I use a Python library `pre-commit` to automatically run all the
checking tools before each commit. Finally I set up a GitHub action to run pre-commit before merging
pull request into master to ultimately enforce the code qualtiy.
...
Python interpreter. Since Poetry manage it for you, run `poetry run which python` to get the
...
the issues. It's really **easy to use**, when you create a new Python project, start it from my
GitHub template. You will only have to run poetry install to get all the dev dependencies required
for the tooling: **linting**, **formating**, **type checking**. You will also have **poetry** to
handle your dependencies and **be ready to ship high-quality code in no time!** Check it out.
