...
will change in the future (e.g. a webpage with a game leaderboard ), you need to remove "perempted"
...
just call the recursive implementation, but they use a cache to avoid re-computing. First
we use the `@cache` decorator, which uses an unbounded cache. Then we use the `@lru_cache` decorator
with a max size of 5. `@lru_cache(maxsize=None)` is equivalent to `@cache`.
...
if __name__ == "__main__":
    ...
    lru_cache_time = lru_cache_timer.t<InfoWarnSection>
  <InfoBlock>
      Costly repeated computations&
      Real-time performace requirements
...
</InfoWarnSection>imeit(number=TIMEIT_REPEAT)
    print(
