...
is a costly type, dependengin on the format (utf-8, ascii, …) we use between 1 and 4 bytes per
...
Let's get our hands dirty on a real dataset, to demonstrate the importance of data types.
...
Then we define two helper functions. `to_mb` just convert a size in bytes (B) to a size in mega
bytes (MB). `analyze` takes a data frame as input and print the data frame dtypes as well as a title
containing the size in MB. To measure a data frame size in memory, can call we use
`df.memory_usage(deep=True)` to get the size per column and sum it to get the total size. The
argument `deep=True` is important to get real amount of memory consumed.
...
The `floor_area_sqm` can fit within a 16 bits float (goes up to **6.55 × 10e4**) column and can be
represented with a `float16`. `resale_price` can fit into a 32 bits unisgned int (goes up to
**4.29e+09**) and can be represented with a `unin32`.

<InfoBlock>Quick tip</InfoBlock>
